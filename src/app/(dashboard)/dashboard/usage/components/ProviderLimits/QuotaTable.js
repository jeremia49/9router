...
export default function QuotaTable({
	...
	// Reset to page 1 when sort mode or quotas change
	const effectiveSort = sortMode;
	const effectiveQuotasLength = quotas.length;
	const shouldResetToFirstPage = useRef(false);

	useEffect(() => {
		shouldResetToFirstPage.current = true;
	}, [effectiveSort, effectiveQuotasLength]);

	useEffect(() => {
		if (shouldResetToFirstPage.current) {
			setPage(1);
			shouldResetToFirstPage.current = false;
		}
	});
	...
	const pageStart =
		sortedQuotas.length === 0 ? 0 : (clampedPage - 1) * PAGE_SIZE + 1;
	const pageEnd = Math.min(clampedPage * PAGE_SIZE, sortedQuotas.length);
