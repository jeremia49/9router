...
describe("openaiToKiroRequest", () => {
	...
	describe("thinking budget", () => {
		it("maps reasoning_effort low to native reasoning fields without legacy prompt tags", () => {
			const body = {
		...
		it("maps reasoning_effort high to native reasoning fields without legacy prompt tags", () => {
			const body = {
		...
		it.each([
			...
			"minimal",
			"ultra",
		])("keeps the legacy thinking fallback for unsupported GPT-5.6 effort %s", (effort) => {
			const body = {
				reasoning: { effort },
			...
			expect(result.additionalModelRequestFields).toBeUndefined();
			expect(systemPromptOf(result)).toContain(
		...
		it.each([
			...
			"off",
			"disabled",
		])("keeps GPT-5.6 reasoning intentionally disabled for effort %s", (effort) => {
			const body = {
				reasoning: { effort },
		...
		it("does not send additionalModelRequestFields for legacy Kiro model ids", () => {
			const body = {
				...
				messages: [
					{
						role: "user",
						content: "Legacy model id should not get adaptive fields",
					},
				],
			};
		...
		it("does not send additionalModelRequestFields for date-suffixed Claude 4 model ids", () => {
			const body = {
				...
				messages: [
					{
						role: "user",
						content: "Date-suffixed Claude 4 should stay legacy",
					},
				],
			};
		...
		it("does not send additionalModelRequestFields for pre-4 legacy Kiro model ids", () => {
			const body = {
				...
				messages: [
					{
						role: "user",
						content: "Older model id should not get adaptive fields",
					},
				],
			};
		...
		it("does not send additionalModelRequestFields for prefixed pre-4 legacy Kiro model ids", () => {
			const body = {
				...
				messages: [
					{
						role: "user",
						content: "Prefixed older model id should not get adaptive fields",
					},
				],
			};
		...
		it("does not send Claude-specific additionalModelRequestFields for prefixed non-Claude aliases", () => {
			const body = {
				...
				messages: [
					{
						role: "user",
						content: "Prefixed non-Claude alias should not get adaptive fields",
					},
				],
			};
		...
		it("does not send Claude-specific additionalModelRequestFields for non-Claude aliases", () => {
			const body = {
				...
				messages: [
					{
						role: "user",
						content: "Non-Claude aliases should not get Claude adaptive fields",
					},
				],
			};
		...
		it("defaults future Kiro model ids to native reasoning fields", () => {
			const body = {
				reasoning_effort: "high",
				messages: [
					{
						role: "user",
						content: "Future model id should get native reasoning fields",
					},
				],
			};
		...
		it("clamps reasoning_effort max to native xhigh reasoning effort", () => {
			const body = {
			...
			expect(systemPromptOf(result)).not.toContain("<max_thinking_length>");
			expect(result.additionalModelRequestFields?.reasoning?.effort).toBe(
				"xhigh",
			);
		});

		it("clamps OpenAI Responses reasoning.effort xhigh to native xhigh reasoning effort", () => {
			const body = {
		...
		it("replays frozen msg0 for explicit Kiro sessions while keeping current time fresh", () => {
			...
			expect(
				second.conversationState.currentMessage.userInputMessage.content,
			).toContain("Current time");
			expect(
				second.conversationState.currentMessage.userInputMessage.content,
		...
		it("does not inject thinking prefix for reasoning_effort none", () => {
			...
			expect(systemPromptOf(result)).not.toContain(
				"<thinking_mode>enabled</thinking_mode>",
			);
			expect(systemPromptOf(result)).not.toContain("<max_thinking_length>");
			expect(result.additionalModelRequestFields).toBeUndefined();
