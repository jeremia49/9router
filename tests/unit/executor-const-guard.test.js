...
describe("OpenCode Free endpoint routing", () => {
	...
	it("declares the Responses format only on the Muse Spark model", () => {
		...
		const muse13 = opencode.models.find(
	...
	it("routes Muse Spark to /responses and every other model to /chat/completions", () => {
		...
		expect(executor.buildUrl(MUSE)).toBe(
			"https://opencode.ai/zen/v1/responses",
		);
		expect(executor.buildUrl(`${MUSE}(xhigh)`)).toBe(
			"https://opencode.ai/zen/v1/responses",
		);
		expect(executor.buildUrl("muse-spark-1.3-contributor-free")).toBe(
			"https://opencode.ai/zen/v1/responses",
		);
		expect(executor.buildUrl("muse-spark-1.4-contributor-free")).toBe(
			"https://opencode.ai/zen/v1/responses",
		);
		expect(executor.buildUrl("muse-spark-2.0-contributor-free(xhigh)")).toBe(
			"https://opencode.ai/zen/v1/responses",
		);
		expect(executor.buildUrl("big-pickle")).toBe(
			"https://opencode.ai/zen/v1/chat/completions",
		);
		expect(executor.buildUrl("hy3-free")).toBe(
			"https://opencode.ai/zen/v1/chat/completions",
		);
	});
