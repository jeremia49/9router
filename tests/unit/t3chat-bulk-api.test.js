...
describe("T3Chat bulk credential parser", () => {
	it("parses valid lines and reports invalid lines", () => {
		const result = parseT3ChatBulkCredentials(
			"cookie-a | convex-a\n\nmissing\n | convex\ncookie-b | convex-b",
		);
		expect(result.valid).toEqual([
			{
				...
				name: "T3Chat 1",
				cookies: "cookie-a",
				convexSessionId: "convex-a",
			},
			{
				line: 5,
				name: "T3Chat 5",
				cookies: "cookie-b",
				convexSessionId: "convex-b",
			},
		]);
		...
		expect(result.valid).toEqual([
			{
				...
				name: "T3Chat 1",
				cookies: "a=b",
				convexSessionId: "c=d | convex",
			},
		]);
		expect(result.invalid).toEqual([
			{ line: 2, reason: "convexSessionId is required" },
		]);
	});
});
...
describe("T3Chat provider APIs", () => {
	...
	it("creates a single T3Chat connection without apiKey", async () => {
		...
		const res = await POST(
			jsonRequest("https://9router.local/api/providers", {
				provider: "t3chat",
				name: "T3 account",
	...
	it("bulk imports valid T3Chat accounts and reports invalid rows", async () => {
		const { POST } = await import(
			"../../src/app/api/providers/t3chat/bulk/route.js"
		);
		const { getProviderConnections } = await import(
			"../../src/models/index.js"
		);
		...
		const res = await POST(
			jsonRequest("https://9router.local/api/providers/t3chat/bulk", {
				text: "cookie-a | convex-a\ninvalid\ncookie-b | convex-b",
			}),
		);
		const body = await res.json();
		const stored = await getProviderConnections({ provider: "t3chat" });
		const imported = stored.filter((connection) =>
			["T3Chat 1", "T3Chat 3"].includes(connection.name),
		);
		...
		expect(
			imported
				.map((connection) => connection.providerSpecificData.convexSessionId)
				.sort(),
		).toEqual(["convex-a", "convex-b"]);
	});
});
