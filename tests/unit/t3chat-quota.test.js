...
describe("T3Chat quota parsing", () => {
  it("extracts tRPC result data from JSON response and JSONL", () => {
    const body = JSON.stringify([{ result: { data: { json: { balance: 10, subTier: "pro" } } } }]);
    expect(extractT3ChatTrpcResult(body)).toEqual({ balance: 10, subTier: "pro" });
    expect(extractT3ChatTrpcResult('{"result":{"data":{"json":{"balance":4}}}}\n')).toEqual({ balance: 4 });
  });

  it("parses customer and subscription data with safe defaults", () => {
    expect(parseT3ChatCustomerData({
      subTier: "pro",
      balance: 12.5,
      lifetimeBalance: 99,
    ...
    expect(parseT3ChatSubscriptionData({ isPaid: true, subTier: "pro" })).toEqual({ isPaid: true, subTier: "pro" });
  });

  it("fetches quota and returns non-blocking error state on failure", async () => {
    const okTransport = {
      get: vi.fn()
        .mockResolvedValueOnce({ status: 200, text: JSON.stringify([{ result: { data: { json: { balance: 7, subTier: "pro" } } } }]) })
        .mockResolvedValueOnce({ status: 200, text: JSON.stringify([{ result: { data: { json: { isPaid: true, subTier: "pro" } } } }]) }),
    };
    const ok = await fetchT3ChatQuota({ cookies: "c=1", transport: okTransport });
    expect(ok.status).toBe("ok");
    expect(ok.data).toMatchObject({ balance: 7, subTier: "pro", isPaid: true });

    const fail = await fetchT3ChatQuota({ cookies: "c=1", transport: { get: vi.fn().mockRejectedValue(new Error("network")) } });
...
describe("T3Chat quota route", () => {
  ...
  it("refreshes quota for one connection and stores providerSpecificData.t3chatQuota", async () => {
    ...
    const { createProviderConnection, getProviderConnectionById } = await import("../../src/models/index.js");
    ...
    const { POST } = await import("../../src/app/api/providers/t3chat/quota/route.js");
    const res = await POST(new Request("https://9router.local/api/providers/t3chat/quota", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
